 ...
 */
fun initBoard(): BoardRun = BoardRun(initialBoardComposition(), Player.WHITE)
...
fun BoardRun.checkCapturesAvailable(): Map<Square, Boolean> {
    return moves.filter { it.value?.player == playerTurn }.mapValues { (square, piece) ->
        piece?.let {
            ...
            directions.any { (rowDiff, colDiff) ->
                val toRow = square.row.index + rowDiff
                val toCol = square.column.index + colDiff
                if (toRow in 0 until BOARD_DIM && toCol in 0 until BOARD_DIM) {
                    val toSquare = Square(Row(toRow), Column(toCol))
                    piece.canCapture(square, toSquare, this)
                } else {
                    false
                }
            }
