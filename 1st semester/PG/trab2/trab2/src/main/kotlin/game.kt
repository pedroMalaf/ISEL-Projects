...
fun Game.moveSpaceship(x: Int) = when {
    ship.canSpaceshipMoveTo(x) -> this.copy(
        ship = Spaceship(
            (x - SHIP_WIDTH / 2),
            shot = ship.shot,
...
fun Game.newAlienShot() = when {
    (0..1).random() == 1 -> this.copy(alienShots = alienShots + addAlienShot(this))
    else -> this
}
...
fun Game.newShipShot() = when {
    ship.canSpaceshipShoot() -> this.copy(ship = ship.addShipShot())
    else -> this
}
